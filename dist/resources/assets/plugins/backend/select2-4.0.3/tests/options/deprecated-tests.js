...
test('converted into dataAdapter.current', function (assert) {
  ...
  data.current(function (data) {
    ...
    assert.equal(item.id, '1', 'The id should have been set by initSelection');

    assert.equal(item.text, '2', 'The text should have been set by initSelection');
  });
...
test('only called once', function (assert) {
  ...
  data.current(function (data) {
    ...
    assert.equal(item.id, '1', 'The id should match the one given by initSelection');

    assert.equal(item.text, '2', 'The text should match the one given by initSelection');
  });
  ...
  data.current(function (data) {
    ...
    assert.equal(item.id, '3', 'The id should match the value given in the DOM');

    assert.equal(item.text, '4', 'The text should match the text given in the DOM');
  });

  assert.equal(called, 1, 'initSelection should have only been called once');
});
...
test('converted into dataAdapter.query automatically', function (assert) {
  ...
  data.query({
    ...
    assert.equal(item.id, 'test', 'The id should have been returned from the query function');

    assert.equal(item.text, 'term', 'The text should have matched the term that was passed in');
  });
...
test('converted ajax-url to ajax--url automatically', function (assert) {
  ...
  assert.ok(options.get('ajax'), 'The `ajax` key was automatically created');
  assert.equal(options.get('ajax').url, 'test://url', 'The `url` property for the `ajax` option was filled in correctly');
});
...
//# sourceMappingURL=deprecated-tests.js.map