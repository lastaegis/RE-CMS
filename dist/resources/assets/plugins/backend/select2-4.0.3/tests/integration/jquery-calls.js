...
test('multiple elements with arguments works', function (assert) {
  ...
  var $first = $('<select>' + '<option>1</option>' + '<option>2</option>' + '</select>');
  ...
  assert.equal($first.val(), '2', 'The call should change the value on the first element');
  assert.equal($second.val(), '2', 'The call should also change the value on the second element');
});
//# sourceMappingURL=jquery-calls.js.map