...
test('createTag returning null does not cut the term', function (assert) {
  ...
  data.on('select', function (params) {
    ...
    assert.equal(params.data.text, 'first, second', 'The text should not have the quotes');
  });
...
test('works with multiple tokens given', function (assert) {
  ...
  assert.equal($select.children('option').length, 3, 'The two new tags should have been created');
});
//# sourceMappingURL=tokenizer-tests.js.map